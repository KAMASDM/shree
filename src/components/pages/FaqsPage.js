...
import { ChevronDown, HelpCircle, Search, LifeBuoy, Shield, Wrench } from 'lucide-react';
...
export default function FaqsPage() {
  ...
  const filteredCategories = useMemo(() => {
    ...
    let allFaqs = categories.flatMap(cat => cat.faqs.map(faq => ({...faq, categoryName: cat.name})));
    ...
    if (activeCategory !== 'All') {
        return [{ name: activeCategory, faqs: allFaqs.filter(faq => faq.categoryName === activeCategory) }];
    }
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-12">
          ...
          <div className="flex flex-wrap justify-center gap-3">
            <button onClick={() => setActiveCategory('All')} className={`px-6 py-2 rounded-full font-semibold text-sm ${activeCategory === 'All' ? 'bg-amber-600 text-white' : 'bg-white text-gray-700'}`}>All</button>
            {categories.map(cat => (
              <button key={cat.id} onClick={() => setActiveCategory(cat.name)} className={`px-6 py-2 rounded-full font-semibold text-sm flex items-center gap-2 ${activeCategory === cat.name ? 'bg-amber-600 text-white' : 'bg-white text-gray-700'}`}>
        ...
        <div className="bg-white p-8 md:p-12 rounded-2xl shadow-lg">
          ...
          {filteredCategories.map((cat, catIndex) => (
            <div key={cat.id}>
              {activeCategory === 'All' && <h2 className="text-2xl font-bold text-gray-800 mt-8 first:mt-0 mb-4">{cat.name}</h2>}
              {cat.faqs.length > 0 ? (
