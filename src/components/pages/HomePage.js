...
const PartnersSection = ({ initialPartners = null }) => {
  const hasInitialData = initialPartners !== null;
...
const FeaturedProductsSection = ({ initialFeaturedProducts = null }) => {
  const hasInitialData = initialFeaturedProducts !== null;
  const [featuredProducts, setFeaturedProducts] = useState(
    Array.isArray(initialFeaturedProducts) ? initialFeaturedProducts : []
  );
  const [loading, setLoading] = useState(!hasInitialData);
  const [error, setError] = useState(null);
...
const ClientsSection = ({ initialClients = null }) => {
  const hasInitialData = initialClients !== null;
...
export default function HomePage({
  ...
  return (
    <>
      <Hero />
      <FeaturedProductsSection initialFeaturedProducts={initialFeaturedProducts} />
      <PartnersSection initialPartners={initialPartners} />
      <WhyChooseUsSection />
      <TargetMarketsSection />
      <UniqueValuePropositionSection />
      <ClientsSection initialClients={initialClients} />
    </>
  );
}
