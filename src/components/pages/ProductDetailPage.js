...
import { 
  ChevronLeft, Star, CheckCircle, Download, 
  Play, Phone, Send, Zap, Shield, Users, Award,
  FileText, Calendar, Settings, AlertTriangle, Heart, Share2
} from 'lucide-react';
...
export default function ProductDetailPage() {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      setLoading(true);
      try {
        const response = await axios.get(`http://localhost:8000/api/products/all/${slug}/`);
  ...
  const galleryImages = [product.main_image, ...(product.images || []).map(img => img.image)];
                ...
                <div className="grid grid-cols-5 gap-4">
                  {galleryImages.map((img, index) => (
                    <button
                      ...
                      style={{
                        ringColor: index === currentImageIndex ? '#b78852' : 'transparent'
                      }}
                    >
                      <img 
                        src={img} 
              ...
              <nav className="flex space-x-8">
                {tabs.map(tab => (
                  <button 
                    ...
                    style={{
                      borderBottomColor: activeTab === tab.key ? '#b78852' : 'transparent',
            ...
            <div>
                  ...
                  >
                    {product.specifications?.map((item, index) => (
                      <div 
                        ...
                        style={{borderBottom: '1px solid rgba(183, 136, 82, 0.1)'}}
                      >
                        <span className="font-medium" style={{color: '#9c7649'}}>{item.spec}:</span>
                        <span className="font-semibold" style={{color: '#8b6a3f'}}>{item.value}</span>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              {activeTab === 'compliance' && (
                ...
                >
                  ...
                  <h3 className="text-lg font-semibold mb-4" style={{color: '#8b6a3f'}}>Compliance Standards</h3>
                  ...
                  >
                    <div className="flex items-start gap-3">
                      <AlertTriangle style={{color: '#b78852'}} className="mt-1" size={20} />
                      <div>
                        <h4 className="font-semibold mb-2" style={{color: '#8b6a3f'}}>Validation Support</h4>
                        <ul className="text-sm space-y-1 list-disc list-inside" style={{color: '#9c7649'}}>
                          {product.documentation?.map((item, index) => <li key={index}>{item.doc}</li>)}
                        </ul>
                      </div>
                    </div>
                  </div>
